...
@onready var editor_skip := OS.has_feature("editor") and Hoarder.first_load and dev_splash_skip
...
func _on_animation_finish(animation_name: StringName):
	...
	if next_scene_is_valid:
		if editor_skip:
			ScreenTransition.auto_transition(ResourceLoader.load_threaded_get(next_scene_path), null)
		else:
			ScreenTransition.auto_transition(ResourceLoader.load_threaded_get(next_scene_path))
	else:
		animation.queue(animation_name)
		transition_out = false
	Hoarder.first_load = false
